...
void ZBuffer::PutObjects(Point3d *vertices, double *colors, int n) {
    int ymax, ymin, ysc, e1, e, i;
    int x[3], y[3];
    //Заносим x,y из vertices в массивы для последующей работы с ними
    for (i = 0; i < n; i++)
    ...
    for (ysc = ymin; ysc < ymax; ysc++) {
        ...
        for (int e = 0; e < 3; e++) {
            ...
            if (ne)
                x2 = x[e] + int (tc * (x[e1] - x[e])),
                        z2 = vertices[e].z + tc * (vertices[e1].z - vertices[e].z);
            else
                x1 = x[e] + int (tc * (x[e1] - x[e])),
                z1 = vertices[e].z + tc * (vertices[e1].z - vertices[e].z),
                ne = 1;
        }
        ...
        for (int xsc = xsc1; xsc < xsc2; xsc++) {
            ...
            if (z < (*(buff[ysc] + xsc)).z)
                for (int i = 0; i < 3; i++) {
                    (*(buff[ysc] + xsc)).color = colors[ysc + i];
                }
            (*( buff[ysc] + xsc)).z = z;
        }
    }
