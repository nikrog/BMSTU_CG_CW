...
lib = ctypes.CDLL('D:/Course_work_CG/srcv_c/libz_buff.dll')
z_buffer = lib.z_buffer
z_buffer.restype = ctypes.c_int
...
class winGL(QtOpenGL.QGLWidget):
    ...
    def __init__(self, parent=None):
        ...
        # текстура
        self.textureSurface = pg.image.load('./img/rock.jpg')
        self.textureData = pg.image.tostring(self.textureSurface, "RGBA", True)
    ...
    def frw_bck_scene(self, st):
        for i in range(len(self.solidRock)):
            self.solidRock[i][0] += st
            self.solidLake[i][0] += st
            self.solidGrass[i][0] += st
            self.solidGrass2[i][0] += st

        for i in range(len(self.solidWater)):
            self.solidWater[i][0] += st

    def up_down_scene(self, st):
        for i in range(len(self.solidRock)):
            self.solidRock[i][1] += st
            self.solidLake[i][1] += st
            self.solidGrass[i][1] += st
            self.solidGrass2[i][1] += st

        for i in range(len(self.solidWater)):
            self.solidWater[i][1] += st
    ...
    def paintSolidObject2(self, vrtxs, indices, image):
        objectVBO = gl.glGenBuffers(1)
        gl.glBindBuffer(gl.GL_ARRAY_BUFFER, objectVBO)
        gl.glBufferData(gl.GL_ARRAY_BUFFER, vrtxs, gl.GL_STATIC_DRAW)
        ...
        gl.glVertexAttribPointer(2, 2, gl.GL_FLOAT, True, 0, None)
    ...
    def paintGL(self):
        ...
        #self.paintSolidObject2(self.solidRock, self.indicesRock, "./img/rockm.PNG")
        #self.paintSolidObject2(self.solidGrass, self.indicesGrass, "./img/grass2.jpg")
        #self.paintSolidObject2(self.solidGrass2, self.indicesGrass2, "./img/grass2.jpg")

        # Установка шейдеров
        shaders = Shader("shader.vs", "shader2.fs")
        shaders.use()
        shaders.setMat4("perspective", self.camera.getProjMatrix())
        shaders.setMat4("view", self.camera.getViewMatrix())
        shaders.setMat4("model", self.object.getModelMatrix())
        ...
        # self.paintSolidObject(self.solidRock, self.indicesRock, self.textureData)
