...
algs = ctypes.CDLL('D:/Course_work_CG/srcv_c/libarr2.dll')
_count_particle_position = algs.count_particle_position
_count_particle_position.restype = ctypes.c_int
_count_particle_position.argtypes = (ctypes.c_double, ctypes.c_double, ctypes.POINTER(ctypes.c_double),
                                     ctypes.POINTER(ctypes.c_double), ctypes.c_int, ctypes.POINTER(ctypes.c_double))


def count_particle_position(sp1, sp2, d1, d2, fl, pos):
    old_dir = (ctypes.c_double * 3)(*d1)
    cur_dir = (ctypes.c_double * 3)(*d2)
    p = (ctypes.c_double * 3)(*pos)
    _count_particle_position(sp1, sp2, old_dir, cur_dir, fl, p)
    return list(p)
...
class Particle:
    directionVariance = [0.2, -0.3, 0.9] #0.2 -0.3 0.1
    ...
    def moveWaterfallParticle(self):
        ...
        if self.vaporized:
            ...
            # пар, который белого цвета
            self.color = glm.vec4(1, 1, 1, 1)

            self.speed -= self.acceleration

            self.pos[0] = self.pos[0] + oldSpeed * oldDirection[0] + (
                        self.speed * self.direction[0]) / 2  # (+ oldSpeed * oldDirection[0])? add 0.5 *
            ...
            #self.pos = count_particle_position(oldSpeed, self.speed, oldDirection, self.direction, 2, self.pos)
        else:
            self.speed += self.acceleration
            ...
            #self.pos = count_particle_position(oldSpeed, self.speed, oldDirection, self.direction, 1, self.pos)
    ...
    # Движение частиц воды по горизонтальной поверхности (скале)
    # (не изменяется координата по Y)
    def moveSolidParticle(self):
        ...
        self.pos[0] = self.pos[0] + oldSpeed * oldDirection[0] + (
                    self.speed * self.direction[0]) / 2  # (+ oldSpeed * oldDirection[0])?
        self.pos[2] = self.pos[2] + oldSpeed * oldDirection[2] + (self.speed * self.direction[2]) / 2

        #self.pos = count_particle_position(oldSpeed, self.speed, oldDirection, self.direction, 0, self.pos)
        self.age += 1
